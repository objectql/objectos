...
export class ObjectDataController {
  ...
  @UseGuards(AuthGuard)
  async query(@Param('objectName') name: string, @Body() dto: QueryDTO, @CurrentUser() user: User) {
    // Validate input
    if (!dto.filters) {
  ...
  @Post(':objectName')
  @UseGuards(AuthGuard)
  async create(@Param('objectName') name: string, @Body() data: any, @CurrentUser() user: User) {
