...
- **Props**:
  - `pageId`: string (e.g., "dashboard-sales") - _The ID of the page definition to load._
  - `context`: Record\<string, any\> - _Optional global variable to pass to widgets (e.g., `{ recordId: "123" }`)_.
- **Behavior**:
...
- **Props**:
  ...
  - `viewName`: string (e.g., "task*kanban") - \_Optional, defaults to object's default view*
  - `initialFilter`: FindOptions - _Optional runtime filters_
- **Behavior**:
  1.  Loads `*.view.yml`.
