...
2. Inject filters during queries
   ...
   class RLSInjector {
     injectFilters(objectName: string, filters: FilterGroup, user: User): FilterGroup;
   }
   ```
...
3. Automatically apply in beforeFind hook
   ...
   kernel.on('beforeFind', async (ctx) => {
     ctx.filters = rlsInjector.injectFilters(ctx.objectName, ctx.filters, ctx.user);
   });
   ```
...
3. Add hook testing utility
   ...
   class HookTester {
     testHook(hookType: HookType, context: HookContext): Promise<HookTestResult>;
   }
   ```
...
2. Implement relationship resolver
   ...
   class LookupResolver {
     async resolve(objectName: string, records: any[], lookupField: string): Promise<any[]>;
   }
   ```
