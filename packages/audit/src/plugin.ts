...
export class AuditLogPlugin implements Plugin {
  ...
  async start(context: PluginContext): Promise<void> {
    ...
    try {
      ...
      if (rawApp) {
        ...
        rawApp.get(
          '/api/v1/audit/field-history/:objectName/:recordId/:fieldName',
          async (c: any) => {
            try {
              const objectName = c.req.param('objectName');
              const recordId = c.req.param('recordId');
  ...
  private async handleDataEvent(event: string, data: any): Promise<void> {
    ...
    this.context?.logger.debug(
      `[Audit Log] Recorded ${eventType} event for ${objectName}/${recordId}`,
    );
  }
  ...
  async applyRetentionPolicy(): Promise<number> {
    ...
    if (retention.defaultRetentionDays && retention.defaultRetentionDays > 0) {
      const cutoff = new Date(
        Date.now() - retention.defaultRetentionDays * 24 * 60 * 60 * 1000,
      ).toISOString();
    ...
    if (totalDeleted > 0) {
      this.context?.logger.info(
        `[Audit Log] Retention cleanup: deleted ${totalDeleted} expired events`,
      );
    }
  ...
  async healthCheck(): Promise<PluginHealthReport> {
    ...
    return {
      ...
      checks: [
        { name: 'audit-storage', status: status === 'healthy' ? 'passed' : 'warning', message },
      ],
    };
  }
        ...
        provides: [
          {
            ...
            methods: [
              { name: 'logEvent', description: 'Record an audit event', async: true },
              {
                ...
                returnType: 'Promise<AuditLogEntry[]>',
                async: true,
              },
              {
                name: 'getTrail',
                description: 'Get audit trail for a record',
                returnType: 'Promise<AuditTrailEntry[]>',
                async: true,
              },
            ],
            stability: 'stable',
