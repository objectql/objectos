...
export class PermissionsPlugin implements Plugin {
  ...
  private async setupTenantIsolation(context: PluginContext): Promise<void> {
    ...
    context.logger.info(
      `[Permissions Plugin] Tenant isolation enabled (field: ${this.config.tenantField})`,
    );
  }
  ...
  private async checkDataPermission(data: any, action: PermissionAction): Promise<void> {
    ...
    const result = await this.engine.checkPermission(permissionContext, objectName, action);
