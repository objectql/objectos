...
import type { SharingRule, SharingAccessLevel, PermissionContext } from './types.js';
...
export class SharingRuleEngine {
   ...
   */
  evaluate(context: PermissionContext, objectName: string): SharingRuleResult {
    const objectRules = this.rules.get(objectName) || [];
    ...
    switch (rule.sharedWithType) {
      ...
      case 'role_and_subordinates':
        ...
        // Check subordinate roles via hierarchy path
        {
          ...
          if (hierarchyPath) {
            return rule.sharedWithValues.some((targetRole) => hierarchyPath.includes(targetRole));
          }
        }
        return false;
