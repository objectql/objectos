...
export class PermissionEngine {
  ...
  async checkPermission(
    ...
    if (!permissionSet) {
      const result = {
        ...
        reason: !this.config.defaultDeny
          ? 'No permission set defined - default allow'
          : 'No permission set defined',
      };
    ...
    for (const profile of context.profiles) {
      ...
      if (actionAllowed) {
        ...
        if (
          profilePermissions.viewFilters &&
          Object.keys(profilePermissions.viewFilters).length > 0
        ) {
    ...
    const result: PermissionCheckResult = {
      ...
      reason: allowed
        ? undefined
        : `No permission for action '${action}' on object '${objectName}'`,
   ...
   */
  private checkActionPermission(permissions: ObjectPermissions, action: PermissionAction): boolean {
    switch (action) {
      case 'create':
   ...
   */
  private replaceTemplateVariables(filters: any, context: PermissionContext): any {
