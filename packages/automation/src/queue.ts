...
export class InMemoryQueue implements Queue {
    ...
    private async processNextBatch() {
        ...
        const pendingJobs = Array.from(this.jobs.values())
            .filter(j => (j.status === 'pending' || j.status === 'delayed') && j.nextRunAt && j.nextRunAt <= now)
        ...
        try {
            ...
            if (job.attempts < job.maxAttempts) {
                ...
                this.logger.info(`[Queue:${this.config.name}] Retrying job ${job.id} in ${delay}ms (Attempt ${job.attempts + 1}/${job.maxAttempts})`);
            } else {
                job.status = 'failed';
                job.failedAt = new Date();
                this.logger.error(`[Queue:${this.config.name}] Job ${job.id} failed permanently after ${job.attempts} attempts`);
            }
        }
    }
}
