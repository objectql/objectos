...
import type {
  IAutomationService,
  AutomationContext as SpecAutomationContext,
  AutomationResult as SpecAutomationResult,
...
export class AutomationPlugin implements Plugin, IAutomationService {
  ...
  async start(context: PluginContext): Promise<void> {
    ...
    for (const rule of rules) {
      if (rule.trigger.type === 'scheduled') {
        this.triggerEngine.registerScheduledTrigger(rule.id, rule.trigger, async () => {
    ...
    if ((this.context as any)?.broker) {
      try {
        ...
        (this.context as any).broker
          .call('data.create', { object: 'automation_log', doc: logData })
          .catch((_e: any) => {
            /* fire and forget */
          }); // fire and forget
      } catch (_ignore) {
   ...
   */
  async calculateFormula(objectName: string, fieldName: string, record: any): Promise<any> {
  ...
  registerFlow(name: string, definition: unknown): void {
    ...
    this.registerRule({ ...rule, id: name }).catch((err) => {
      this.context?.logger.warn(
        `[Automation Plugin] Failed to register flow ${name}: ${err instanceof Error ? err.message : err}`,
      );
    });
  }
  ...
  unregisterFlow(name: string): void {
    if (this.storage.deleteRule) {
      this.storage.deleteRule(name).catch((err: unknown) => {
        this.context?.logger.warn(
          `[Automation Plugin] Failed to unregister flow ${name}: ${err instanceof Error ? err.message : err}`,
        );
      });
    }
  }
  ...
  async healthCheck(): Promise<PluginHealthReport> {
    ...
    return {
      ...
      checks: [
        { name: 'automation-engine', status: status === 'healthy' ? 'passed' : 'warning', message },
      ],
    };
  }
        ...
        provides: [
          {
            ...
            methods: [
              ...
              {
                name: 'evaluateRules',
                description: 'Evaluate rules for a trigger event',
                async: true,
              },
