...
import { AutomationPlugin, getAutomationAPI } from '../src/plugin.js';
...
describe('Automation Plugin', () => {
  ...
  describe('Plugin Lifecycle', () => {
    ...
    it('should start successfully', async () => {
      ...
      expect(mockContext.logger.info).toHaveBeenCalledWith(expect.stringContaining('Starting'));
      expect(mockContext.logger.info).toHaveBeenCalledWith(
  ...
  describe('Rule Execution', () => {
    ...
    it('should execute rule actions on trigger', async () => {
      ...
      expect(mockUpdateHandler).toHaveBeenCalledWith('Contact', '123', { status: 'new' });
  ...
  describe('Formula Fields', () => {
    ...
    it('should throw error for non-existent formula', async () => {
      ...
      await expect(api!.calculateFormula('Order', 'nonExistent', {})).rejects.toThrow(
        'Formula not found: Order.nonExistent',
      );
    });
  });
