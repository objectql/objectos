...
describe('NotificationService', () => {
  ...
  beforeEach(() => {
    ...
    mockContext = {
      ...
      getData: vi.fn((key?: string) =>
        key ? mockContext.instance.data[key] : mockContext.instance.data,
      ),
      setData: vi.fn(),
    } as any;
  });
  ...
  describe('send', () => {
    ...
    it('should handle errors from notification handlers', async () => {
      ...
      await expect(notificationService.send(config, mockContext)).rejects.toThrow(
        'Notification failed',
      );
    });
  });
...
describe('WebhookNotificationHandler', () => {
  ...
  describe('send', () => {
    ...
    it('should handle multiple webhook URLs', async () => {
      const config: NotificationConfig = {
        channel: 'webhook',
        recipients: ['https://example.com/webhook1', 'https://example.com/webhook2'],
        data: { event: 'workflow.completed' },
      };
