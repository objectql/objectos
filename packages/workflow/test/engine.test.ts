...
describe('WorkflowEngine', () => {
  ...
  describe('startInstance', () => {
    ...
    it('should throw error if trying to start non-pending workflow', async () => {
      ...
      await expect(engine.startInstance(instance, definition)).rejects.toThrow(
        'Cannot start workflow in status: running',
      );
    });
  });
  ...
  describe('executeTransition', () => {
    it('should execute a valid transition', async () => {
      ...
      const updated = await engine.executeTransition(instance, definition, 'submit', 'user-1');
    ...
    it('should throw error for invalid transition', async () => {
      ...
      await expect(engine.executeTransition(instance, definition, 'approve')).rejects.toThrow(
        'Transition "approve" not available in state "draft"',
      );
    });
    ...
    it('should throw error when executing transition on non-running workflow', async () => {
      ...
      await expect(engine.executeTransition(instance, definition, 'submit')).rejects.toThrow(
        'Cannot execute transition on workflow in status: pending',
      );
    });
    ...
    it('should mark workflow as completed when reaching final state', async () => {
      ...
      const completed = await engine.executeTransition(instance, definition, 'approve', 'user-1');
  ...
  describe('abortInstance', () => {
    ...
    it('should throw error when aborting non-running workflow', async () => {
      ...
      await expect(engine.abortInstance(instance, definition)).rejects.toThrow(
        'Cannot abort workflow in status: pending',
      );
    });
  });
  ...
  describe('guard conditions', () => {
    it('should block transition when guard returns false', async () => {
      ...
      await expect(engine.executeTransition(instance, definition, 'submit')).rejects.toThrow(
        'Transition "submit" blocked by guard conditions',
      );
  ...
  describe('canExecuteTransition', () => {
    it('should return true for valid transition', async () => {
      ...
      const canExecute = await engine.canExecuteTransition(instance, definition, 'submit');
