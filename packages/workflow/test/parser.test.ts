...
`;
      ...
      expect(() => parseWorkflowYAML(yaml)).toThrow(
        'Workflow definition must have at least one state',
      );
    });
...
`;
      ...
      expect(() => parseWorkflowYAML(yaml)).toThrow(
        'Workflow definition must have an initial state',
      );
    });
...
`;
      ...
      expect(() => parseWorkflowYAML(yaml)).toThrow('target state "nonexistent" does not exist');
    });
...
`;
      ...
      expect(() => parseWorkflowYAML(yaml)).toThrow('Transition "next" must have a target state');
    });
  });
...
describe('validateWorkflowDefinition', () => {
  ...
  it('should detect invalid transition target', () => {
    ...
    expect(errors.some((e) => e.includes('target state "nonexistent" does not exist'))).toBe(true);
  });
});
