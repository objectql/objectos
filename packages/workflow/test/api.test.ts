...
describe('WorkflowAPI', () => {
  ...
  describe('startWorkflow', () => {
    ...
    it('should start a new workflow instance', async () => {
      const instance = await api.startWorkflow('test-workflow', { foo: 'bar' }, 'user-1');
    ...
    it('should throw error for non-existent workflow', async () => {
      await expect(api.startWorkflow('non-existent')).rejects.toThrow(
  ...
  describe('executeTransition', () => {
    it('should execute a transition successfully', async () => {
      ...
      const updated = await api.executeTransition(instance.id, 'submit', 'user-1', {
    ...
    it('should throw error for non-existent instance', async () => {
      await expect(api.executeTransition('non-existent', 'submit')).rejects.toThrow(
  ...
  describe('abortWorkflow', () => {
    ...
    it('should throw error for non-existent instance', async () => {
      await expect(api.abortWorkflow('non-existent')).rejects.toThrow(
        'Workflow instance not found: non-existent',
      );
    });
  ...
  describe('getAvailableTransitions', () => {
    ...
    it('should throw error for non-existent instance', async () => {
      await expect(api.getAvailableTransitions('non-existent')).rejects.toThrow(
        'Workflow instance not found: non-existent',
      );
    });
  });
  ...
  describe('task management', () => {
    ...
    it('should throw error when completing non-pending task', async () => {
      ...
      await expect(api.completeTask(task.id)).rejects.toThrow(
        'Cannot complete task in status: completed',
      );
    });
    ...
    it('should throw error when rejecting non-pending task', async () => {
      ...
      await expect(api.rejectTask(task.id)).rejects.toThrow(
        'Cannot reject task in status: rejected',
      );
    });
  });
  ...
  describe('escalateTask', () => {
    ...
    it('should throw error for non-existent task', async () => {
      await expect(api.escalateTask('non-existent', 'director@example.com')).rejects.toThrow(
        'Task not found',
      );
    });
    ...
    it('should throw error when escalating completed task', async () => {
      ...
      await expect(api.escalateTask(task.id, 'director@example.com')).rejects.toThrow(
        'Cannot escalate task in status: completed',
      );
    });
    ...
    it('should throw error when escalating rejected task', async () => {
      ...
      await expect(api.escalateTask(task.id, 'director@example.com')).rejects.toThrow(
        'Cannot escalate task in status: rejected',
      );
    });
  });
