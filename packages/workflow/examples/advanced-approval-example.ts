...
async function main() {
  ...
  const submitted = await api.executeTransition(instance.id, 'submit', 'employee@company.com');
  ...
  if (tasks.length > 0) {
    ...
    console.log(
      `Task ${delegated.id} delegated from ${delegated.originalAssignee} to ${delegated.delegatedTo}`,
    );
  ...
  const finalApproved = await api.executeTransition(instance.id, 'approve', 'cfo@company.com');
