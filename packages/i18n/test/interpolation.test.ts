...
describe('Interpolation', () => {
  ...
  describe('processDirectives', () => {
    it('should process number directive', () => {
      const result = processDirectives('Total: {{value | number}}', { value: 1234.56 }, 'en-US');
    ...
    it('should process date directive', () => {
      const testDate = new Date('2024-01-15T12:30:00Z');
      const result = processDirectives('Published: {{date | date}}', { date: testDate }, 'en-US');
      expect(result).toContain('Jan');
    });

    it('should handle mixed directives and regular interpolation', () => {
      const result = processDirectives(
        'Hello {{name}}, balance: {{amount | currency:USD}}',
    ...
    it('should preserve non-matching patterns', () => {
      const result = processDirectives('{{value | unknown}}', { value: 123 }, 'en-US');
