...
export const getBetterAuth = async (config: BetterAuthConfig = {}) => {
  ...
  try {
    ...
    const resolveProvider = (
      ...
      const secret =
        cfgObj?.clientSecret || legacySecret || process.env[`${name.toUpperCase()}_CLIENT_SECRET`];
          ...
          create: {
            before: async (user) => {
              try {
                ...
                console.log(
                  `[Better-Auth Plugin] Creating user with role: ${role} (current count: ${count})`,
                );
    ...
    console.log(
      `[Better-Auth Plugin] Initialized successfully (providers: ${enabledProviders.join(', ') || 'email-only'})`,
    );
    return authInstance;
  } catch (e: any) {
