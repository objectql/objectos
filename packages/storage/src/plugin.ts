...
import type {
  IStorageService,
  StorageUploadOptions as SpecStorageUploadOptions,
  StorageFileInfo as SpecStorageFileInfo,
...
export class StoragePlugin implements Plugin, IStorageService {
  ...
  private fileMeta: Map<
    string,
    { size: number; contentType?: string; metadata?: Record<string, string>; lastModified: Date }
  > = new Map();
  ...
  async upload(
    ...
    options?: SpecStorageUploadOptions,
  ): Promise<void> {
    // ReadableStream conversion uses global Response API (available in Node.js 18+ LTS)
    const buf = Buffer.isBuffer(data)
      ? data
      : Buffer.from(await new Response(data as any).arrayBuffer());
        ...
        provides: [
          {
            ...
            methods: [
              ...
              {
                name: 'upload',
                description: 'Upload file to storage (IStorageService)',
                async: true,
              },
              {
                name: 'download',
                description: 'Download file from storage (IStorageService)',
                returnType: 'Promise<Buffer>',
                async: true,
              },
              {
                name: 'exists',
                description: 'Check if file exists (IStorageService)',
