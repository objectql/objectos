...
import type { Job, JobConfig, JobStorage } from './types.js';
...
export class JobScheduler {
  ...
  constructor(options: { storage: JobStorage; queue: JobQueue; logger?: any }) {
  ...
  async scheduleJob(config: JobConfig): Promise<Job> {
    ...
    this.logger.info(
      `[Scheduler] Scheduled job: ${job.id} (${job.name}) - Next run: ${nextRun.toISOString()}`,
    );
  ...
  async updateSchedule(jobId: string, cronExpression: string): Promise<void> {
    ...
    this.logger.info(
      `[Scheduler] Updated schedule for ${jobId} - Next run: ${nextRun.toISOString()}`,
    );
  }
}
