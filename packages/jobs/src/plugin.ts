...
import type {
  IJobService,
  JobSchedule as SpecJobSchedule,
  JobHandler as SpecJobHandler,
  JobExecution as SpecJobExecution,
...
import { createDataCleanupJob, createReportJob, createBackupJob } from './built-in-jobs.js';
...
export class JobsPlugin implements Plugin, IJobService {
    ...
    if (!this.config.storage) {
      if (this.config.persistence === 'persistent') {
        ...
        try {
          ...
          context.logger.warn(
            '[Jobs Plugin] Persistent storage requested but storage service unavailable, falling back to memory',
          );
        }
      }
  ...
  async schedule(
    nameOrConfig: string | JobConfig,
    specSchedule?: SpecJobSchedule,
    handler?: SpecJobHandler,
  ): Promise<Job | void> {
  ...
  async healthCheck(): Promise<PluginHealthReport> {
    ...
    try {
      stats = await this.queue.getStats();
    } catch {
      /* ignore */
    }
    const message = stats
            ...
            methods: [
              {
                ...
                description: 'Register a custom job handler',
                async: false,
              },
              {
                name: 'enqueue',
                description: 'Add a job to the queue',
                returnType: 'Promise<Job>',
                async: true,
              },
              {
                name: 'schedule',
              ...
              },
              {
                ...
                returnType: 'Promise<JobQueueStats>',
                async: true,
              },
              {
                name: 'enqueueBatch',
                description: 'Enqueue a batch of jobs',
                returnType: 'Promise<Job[]>',
                async: true,
              },
            ],
