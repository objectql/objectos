...
describe('JobQueue', () => {
  ...
  describe('registerHandler', () => {
    ...
    it('should throw error for duplicate handler', () => {
      ...
      expect(() => queue.registerHandler(definition)).toThrow(
        'Job handler "test-job" is already registered',
      );
    });
  });
  ...
  describe('cancel', () => {
    ...
    it('should throw error when cancelling non-existent job', async () => {
      await expect(queue.cancel('non-existent')).rejects.toThrow('Job non-existent not found');
    });
  });
