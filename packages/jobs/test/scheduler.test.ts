...
describe('JobScheduler', () => {
  ...
  describe('unscheduleJob', () => {
    ...
    it('should throw error for non-scheduled job', async () => {
      ...
      await expect(scheduler.unscheduleJob('job-1')).rejects.toThrow('Job job-1 is not scheduled');
    });
  });
  ...
  describe('updateSchedule', () => {
    ...
    it('should throw error for invalid cron expression', async () => {
      ...
      await expect(scheduler.updateSchedule('job-1', 'invalid')).rejects.toThrow(
        'Invalid cron expression',
      );
    });

    it('should throw error for non-scheduled job', async () => {
      await expect(scheduler.updateSchedule('non-existent', '0 0 * * *')).rejects.toThrow(
        'Job non-existent not found',
      );
    });
  });
